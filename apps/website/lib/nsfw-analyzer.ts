...
export interface NSFWAnalysisResult {
  isNsfw: boolean
  class: (typeof NSFW_CLASSES)[keyof typeof NSFW_CLASSES]
  scores: {
    safe: number
    suggestive: number
    nudity: number
...
async function analyzeLocalNSFW(buffer: Buffer, mimeType: string, filename?: string): Promise<NSFWAnalysisResult> {
  ...
  let nsfw = false
  let detectedClass: NSFWAnalysisResult["class"] = NSFW_CLASSES.SAFE
...
export async function detectNSFWContent(
  ...
  try {
    ...
    if (googleResult.isNsfw) {
      const googleScores = googleResult.scores
      return {
        ...googleResult,
        class: localResult.class,
        scores: googleScores
          ? {
              safe: googleScores.safe,
              suggestive: localResult.scores.suggestive,
              nudity: googleScores.nudity,
              sexual_activity: googleScores.sexual_activity,
              pornographic: localResult.scores.pornographic,
            }
          : localResult.scores,
        method: "combined",
        details: {
