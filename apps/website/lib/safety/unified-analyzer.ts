...
export async function analyzeContent(
  ...
  if (contentType === "video") {
    ...
    try {
      ...
      if (googleResult.isNsfw) {
        flags.full_nudity = (googleResult.scores?.nudity ?? 0) > 0.5
        flags.explicit_sexual_activity = (googleResult.scores?.sexual_activity ?? 0) > 0.4
        scores.nudity = googleResult.scores?.nudity ?? 0
        scores.sexual_activity = googleResult.scores?.sexual_activity ?? 0
        scores.safe = 0
  ...
  flags = {
    ...
    violence: violenceScores.violence > 0.75 || flags.violence,
    gore: violenceScores.gore > 0.75 || flags.gore,
  }
  ...
  if (customResult.isNsfw && customResult.confidence > 0.7) {
    ...
    scores.nudity = Math.max(scores.nudity, customResult.scores.nudity)
    scores.sexual_activity = Math.max(scores.sexual_activity, customResult.scores.sexual_activity)
    scores.safe = Math.min(scores.safe, customResult.scores.safe)
  }

  // Step 3: Google Vision fallback if uncertain
  if (customResult.confidence > 0.3 && customResult.confidence < 0.7) {
    console.log("[v0] Custom detector uncertain - using Google Vision")
    try {
      const googleResult = await detectAdultContentWithGoogle(buffer)
      if (googleResult.isNsfw) {
        scores.nudity = Math.max(scores.nudity, googleResult.scores?.nudity ?? 0.8)
        scores.sexual_activity = Math.max(scores.sexual_activity, googleResult.scores?.sexual_activity ?? 0.6)
        scores.safe = Math.min(scores.safe, googleResult.scores?.safe ?? 0.2)
      }
    } catch (error) {
