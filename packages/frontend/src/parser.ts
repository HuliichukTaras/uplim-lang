  ...
  private parseVariableDeclaration(): VariableDeclaration {
    ...
    if (this.match(TokenType.LBRACE)) {
      ...
      if (this.current().type !== TokenType.RBRACE) {
        do {
          const keyToken = this.consume(TokenType.IDENTIFIER, "Expected property name in object pattern")
          let value = keyToken.value

          if (this.match(TokenType.COLON)) {
            value = this.consume(TokenType.IDENTIFIER, "Expected binding name in object pattern").value
          }
      ...
      if (this.current().type !== TokenType.RBRACKET) {
        do {
          elements.push(this.consume(TokenType.IDENTIFIER, "Expected identifier in array pattern").value)
        } while (this.match(TokenType.COMMA))
      }
